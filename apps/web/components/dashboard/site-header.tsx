		...
		<header className="relative z-10 flex h-(--header-height) shrink-0 items-center gap-2 transition-[width,height] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:h-(--header-height)">
			<div className="flex w-full justify-between px-6 sm:px-8 mt-16">
				...
				<div className="flex items-center gap-1">
					<Link
						href="/dashboard"
						className="inline-flex items-center justify-center size-9 rounded border bg-background shadow-xs hover:bg-accent hover:text-accent-foreground dark:bg-input/30 dark:border-input dark:hover:bg-input/50 text-black dark:text-white"
					>
						<HouseIcon size={26} weight="thin" color="currentColor" />
					</Link>
					<Link
						href="/dashboard/billing"
						className="inline-flex items-center justify-center size-9 rounded border bg-background shadow-xs hover:bg-accent hover:text-accent-foreground dark:bg-input/30 dark:border-input dark:hover:bg-input/50 text-black dark:text-white"
					>
						<WalletIcon size={26} weight="thin" color="currentColor" />
					</Link>
					<Link
						href="/dashboard/settings"
						className="inline-flex items-center justify-center size-9 rounded border bg-background shadow-xs hover:bg-accent hover:text-accent-foreground dark:bg-input/30 dark:border-input dark:hover:bg-input/50 text-black dark:text-white"
					>
						<FadersIcon size={26} weight="thin" color="currentColor" />
					</Link>
					<button
						...
						onClick={logout}
					>
						<SignOutIcon size={26} weight="thin" color="currentColor" />
					</button>
				</div>
			</div>
		</header>
