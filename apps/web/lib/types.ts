...
export const outlineSchema = z.object({
	...
	cachedName: z
		.string()
		.optional()
		.describe("Optional cached content name for context caching"),
	hasRefs: z
		.boolean()
		.describe("Does the document have any references or citations?"),
	sections: z
		.array(
			z.object({
				...
				content: z
					.string()
					.describe(
						"A prompt for the next agent in the chain to generate the content for this section",
					),
			}),
		)
	...
	references: z
		.array(z.string())
		.describe("Any references or citations for the document"),
});
